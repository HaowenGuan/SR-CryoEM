...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding=1):
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            assert num_channels % num_groups == 0, "Expected number of channels in input to be divisible by num_groups. num_channels={num_channels}, num_groups={num_groups}"
...
class firstEncoder(nn.Module):
    def __init__(self, in_channels, out_channels, conv_kernel_size=3,num_groups=8):
        super(firstEncoder, self).__init__()
        if in_channels < num_groups:
            num_groups = 1
        self.gn_firstencoder = nn.GroupNorm(num_groups=num_groups, num_channels=in_channels)
        self.conv3d_firstencoder = conv3d(in_channels, out_channels, 3, bias=False, padding=1)
        self.reslu_firstencoder = nn.ReLU(inplace=True)

    def forward(self, x):
        x = self.reslu_firstencoder(self.conv3d_firstencoder(self.gn_firstencoder(x)))
        return x
